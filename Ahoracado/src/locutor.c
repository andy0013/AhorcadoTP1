...
void locutor_recibir_y_actualizar(locutor *locutor, protocolo_t *protocolo){
	...
	protocolo_recibir_datos_partida_servidor(protocolo,&locutor->termino_la_partida, &locutor->intentos_usuario);

	protocolo_recibir_datos_longitud_palabra_servidor(protocolo,&locutor->longitud_palabra);

	char palabra_de_partuda[locutor->longitud_palabra];

	protocolo_recibir_datos_palabra_servidor(protocolo,&palabra_de_partuda,&locutor->longitud_palabra);
...
void locutor_solicitar_letra_del_input_user(locutor *locutor, char *letra_a_enviar){
	...
	if(locutor->input_user == NULL){
		...
		*letra_a_enviar = locutor->input_user[locutor->posicion_de_letra_enviada];
