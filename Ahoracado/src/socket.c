 ...
 */
static struct addrinfo *_prepare_getaddrinfo(socket_t *self, const char *host, const char *service, int flags) {
...
int socket_connect(socket_t *self, const char *host, const char *service){
	...
	int fd = -1;
	for (ptr = addr_result; ptr != NULL && connection_err == 0; ptr = ptr->ai_next) {
		/* Creamos el socket definiendo la familia (deberia ser AF_INET IPv4),
		 el tipo de socket (deberia ser SOCK_STREAM TCP) y el protocolo (0) */
		fd = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
...
ssize_t socket_send(socket_t *self, const char *buffer, size_t length){
    ...
    while (all_bytes_sent < length) {
        bytes = send(self->fd, &send_request[all_bytes_sent],
                     bytes_not_yet, MSG_NOSIGNAL);
        if (bytes == -1) {
            all_bytes_sent = -1;
            break;
        }
